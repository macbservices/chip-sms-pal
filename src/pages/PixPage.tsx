...
const PixPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (v >= 5 && v <= 1000) {
      await supabase.from('profiles').update({ saldo: profile.saldo + v }).eq('id', user.id);
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="number" step="0.01" min="5" max="1000" value={valor}
              onChange={(e) => setValor(e.target.value)} placeholder="10.00" required
